...
class MultiModalDataset(Dataset):
    ...
    def _get_img_cam1(self, demo_idx, frame_idx, j, k):
        # max_frame_idx = self.frames_per_demo[demo_idx] - 1
        # j = random.randint(frame_idx + 1, max_frame_idx - 1)
        # k = random.randint(j + 1, max_frame_idx)
        imgs = {
            ...
            "cam1_j": self._get_img("cam1", demo_idx, j),
            "cam1_k": self._get_img("cam1", demo_idx, k),
        }
        return imgs

    def _get_img_cam2(self, demo_idx, frame_idx, j, k):
        # max_frame_idx = self.frames_per_demo[demo_idx] - 1
        # j = random.randint(frame_idx + 1, max_frame_idx - 1)
        # k = random.randint(j + 1, max_frame_idx)
        imgs = {
            ...
            "cam2_j": self._get_img("cam2", demo_idx, j),
            "cam2_k": self._get_img("cam2", demo_idx, k),
        }
        return imgs

    def _get_img_cam3(self, demo_idx, frame_idx, j, k):
        # max_frame_idx = self.frames_per_demo[demo_idx] - 1
        # j = random.randint(frame_idx + 1, max_frame_idx - 1)
        # k = random.randint(j + 1, max_frame_idx)
        imgs = {
            ...
            "cam3_j": self._get_img("cam3", demo_idx, j),
            "cam3_k": self._get_img("cam3", demo_idx, k),
        }
        return imgs

    def _get_ee_pose(self, demo_idx, frame_idx, j, k):
    ...
    def _get_tactile(self, demo_idx, frame_idx, j, k):
        ...
        tactile = {
            ...
            "tactile_j": tactile_array[j],
            "tactile_k": tactile_array[k],
        }
        return tactile

    def _get_act_chunk(self, demo_idx, start_idx, j, k):
    ...
    def _get_act_mask(self, demo_idx, start_idx, j, k):
        chunk_size = self.chunk_size
    ...
    def __getitem__(self, index):
        ...
        for key in self.keys:
            item.update(self._fetch_val_fmap[key](demo_idx, frame_idx, j, k))
        return item
...
def build_obs_dict(args, keys, batch):
    ...
    obs = {"curr": [], "next": [], "goal": [], "j": [], "k": [], "ee": None}
