...
def train_one_epoch(
    ...
    for it, batch in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        batch_size = args.batch_size_per_gpu
        # Shape of stacked_tensor = (3, batch_size, 3, 224, 224)
        stacked_tensor = torch.stack(obs["curr"], dim=0)
        indices = torch.randperm(batch_size)
        random_curr = stacked_tensor[:, indices]
        # random_curr = [tensor[indices] for tensor in obs["curr"]]
        # print("\nShuffled Tensor:")
        # print(random_curr.shape)
...
def validate(
    ...
    for it, batch in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        stacked_tensor = torch.stack(obs["curr"], dim=0)
        indices = torch.randperm(batch_size)
        # Apply the permutation to shuffle columns (entire rows remain consistent)
        random_curr = stacked_tensor[:, indices]
        obs_j = obs["j"]
        obs_k = obs["k"]
