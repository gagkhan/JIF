...
class PushTImageDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # nsample['agent_pos_next'] = torch.tensor(train_data['agent_pos'][indices['o_next']], dtype=torch.float32)
...
def get_vitact(use_tactile) -> nn.Module:
    ...
    encoder_args = argparse.Namespace(
        ...
        patch_size  =None,
        drop_path_rate=0.1)
    # checkpoint = torch.load("checkpoint_stage1_human_TCL.pth", map_location="cpu")
    # encoder_args = checkpoint["args"]
    # state_dict = {k.replace("module.", ""): v for k, v in checkpoint["student"].items() if "module." in k}
